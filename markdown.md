...
As previously mentioned, markdown can be converted to HTML for a nice static web page (or parts of it). To do so, you could use CLI command `markdown` or use online markdown converter.

To use `markdown` in CLI, you may need to install it first. Running the command with the path to the markdown file as an arguement simply routes the HTML code in plaintext to teh stdout. You could simply reroute the output to a file by using '>'. As for online tools, there are a phletora of websites which provides markdown conversion services, such as [Free Markdown to HTML Converter](https://markdowntohtml.com/), [CloudConvert](https://cloudconvert.com/md-to-html), and [browserling](https://www.browserling.com/tools/markdown-to-html).
